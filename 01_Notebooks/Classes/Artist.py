...
class Artist:
    def __init__(self, display_name, nationality=None, wikidata_uri=None, date_of_birth=None): #defining the class and its parameters (parameters found in the MET dataset but also through enritchment)
    ...
    @classmethod
    def from_dataframe(cls, df: pd.DataFrame, index: int): #creates an artist from one row of the dataframe using the parameters defined above, by their index in the dataframe and the name of the column
    ...
    def to_rdf(self): #creates a rdf-graph for artist 
        ...
        CRM = Namespace("http://www.cidoc-crm.org/cidoc-crm/") #defining the ontologies 
        DC = Namespace("http://purl.org/dc/elements/1.1/")
        EX = Namespace("http://w3id.org/example/")
        FOAF = Namespace("http://xmlns.com/foaf/0.1/")
        Schema = Namespace("http://schema.org/")

        g.bind("crm", CRM) #binding the abbreviation to the graph so when we serialize it, it will use #crm:something instead of the full namespace
        g.bind("dc", DC)
        g.bind("ex", EX)
        ...
        artist_uri = URIRef(self.wikidata_uri) #creates a artist_uri out of the wikidata_uri

        g.add((artist_uri, RDF.type, CRM["E20_Person"])) #adds triples using the ontologies 
        g.add((artist_uri, FOAF.name, Literal(self.display_name)))
        g.add((artist_uri, Schema.nationality, Literal(self.nationality)))
        
        if self.date_of_birth: 
            g.add((artist_uri, Schema.birthDate, Literal(self.date_of_birth))) #adds a birth date if there is one 
    ...
    def wikidata_enrich(self): #enriches the class information using a SPARQL querie to wikidata 
  
        if not isinstance(self.wikidata_uri, str): #skips the enrichment if the data is not a string and prints an answer 
        ...
        try: #sends the query to wikidata to look if there is any infomation that was asked for
            ...
            for result in results["results"]["bindings"]: #if there is information it gets saved 
